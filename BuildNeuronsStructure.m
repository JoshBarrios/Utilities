...
fishNums = str2double(fluoDataAll(:,1));
planeNums = str2double(fluoDataAll(:,2));
trialNums = str2double(fluoDataAll(:,3));
...
for k = 1:numFish
    for l = 1:numPlanes
        ...
        ROIs = UnpackCellofCells(thisPlaneROIs);
        ...
        trialNums = ones(size(thisPlaneRasters{1},2),1);
        for m = 2:size(thisPlaneRasters,1)
            num = size(thisPlaneRasters{m},2);
