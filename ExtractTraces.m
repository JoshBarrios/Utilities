%% rois contains the coordinates of the points that make up each ROI
%  traces is the matrix of dF/F traces over time for each ROI
%  Y is your raw movie
%  path is the path of the folder containing the images. This will be used to save variables at the end.
%  You can get this easily using the uigetdir command, e.g. "path = uigetdir;"
% This code requires the folder "ca_source_extraction-master" from the Pnevmatikakis et al supplement

function [rois,traces,spikes] = ExtractTraces(Y,path,numCells)
...
K = numCells;                                     % number of components to be found
...
saveas(gcf,fullfile(path,'ROIs.pdf'))
traces = full(C_df)';
rois = Coor;
spikes = S_or;
save(fullfile(path,'spikes.mat'),'spikes')
save(fullfile(path,'traces.mat'),'traces')
save(fullfile(path,'rois.mat'),'rois')
